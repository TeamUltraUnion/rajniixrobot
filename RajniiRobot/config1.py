...
def get_user_list(config, key):
    with open('{}/RajniiRobot/{}'.format(os.getcwd(), config),
              'r') as json_file:
...
class Config(object):
    ...
    DRAGONS = [
        1118151835
    ]  # ID's of users whom you want give Global Banning & SUDO Rights (SUDO have less rights then developers and owner), also known as sudo users.
    DEV_USERS = [
        5205895700, 5262453320
    ]  # ID's of users who can use shell, and test codes known as devs.
    DEMONS = [
        2116934109
    ]  # ID's of users who can ban and mute users globally by your bot, they can't use all sudo rights, also known as Support Staff Members.
    WOLVES = [
    ]  # ID's of users who just cannot be restricted by your bot through ban, mute, warn and global ban, mute commands, known as Immune Users.
    TIGERS = [
    ]  # ID's of users who can use warn, mute, ban and admin commands through your bot, are Immune from all restriction commands of your bot.
    "Required - [V-D/C]"
    ...
    HEROKU_APP_NAME = 'rajniixrobot2'  # Heroku App Name, if you're using Heroku.com for deployment of this bot.
    HEROKU_API_KEY = "4c25e3a6-17bc-4acd-bfec-acaf3594f96b"  # True or False, if you are using Heroku.com for deployment, get it from heroku account info; otherwise leave it empty.
